...
class MyApp extends StatelessWidget {
  // This widget is the root of your application.
  Widget getMainPage() {
    Widget mainPage = SignIn();

    if (html.window.sessionStorage.isNotEmpty &&
        html.window.sessionStorage['email'] != "" &&
        html.window.sessionStorage['courseCode'] != "") {
      if (html.window.sessionStorage['IsAdmin'] != null &&
          html.window.sessionStorage['IsAdmin'].toString() == "1") {
        mainPage = AdminReports();
      } else {
        mainPage = Home();
      }
    }
